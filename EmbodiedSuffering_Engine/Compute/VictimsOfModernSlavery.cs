...
namespace BH.Engine.EmbodiedSuffering
{
    public static partial class Compute
    {
        ...
        public static Output<double, List<Country>, double> VictimsOfModernSlavery(List<Material> materials, List<double> ratios, Country importCountry = Country.UnitedStatesOfAmerica, ImportSourceType importSourceType = ImportSourceType.ByMass)
        {
            ...
            return VictimsOfModernSlavery(importSources, ratios);
        }
        ...
        public static Output<double, List<Country>, double> VictimsOfModernSlavery(List<MaterialImportSources> materialImportSources, List<double> ratios)
        {
            ...
            if (ratiosSum == 0)
            {
                Engine.Base.Compute.RecordError("Total ratios are summing to 0. Unable to compute VictimsOfModernSlavery value. Raturning NaN.");
                return new Output<double, List<Country>, double> { Item1 = double.NaN, Item2 = new List<Country>(), Item3 = 0 };
            }
            ...
            {
                Output<double, List<Country>, double> singleItemFreedom = materialImportSources[i].VictimsOfModernSlavery();
        ...
        public static Output<double, List<Country>, double> VictimsOfModernSlavery(this MaterialImportSources materialImportSource)
        {
            ...
            Dictionary<Country, double> victimsOfModernSlaverys = Query.GlobalSlaveryIndexDictionary();
            ...
            for (int i = 0; i < materialImportSource.ExportCountries.Count; i++)
            {
                ...
                double victimsOfSlavery;
                if (!victimsOfModernSlaverys.TryGetValue(exportCountry, out victimsOfSlavery))
                {
                    missingRatio += ratio;
            ...
            if (totalRatioUsed == 0)
            {
                Base.Compute.RecordError($"No victims of modern slavery values found for any of the export countries in the provided {nameof(MaterialImportSources)} for the import country {materialImportSource.ImportCountry} for material of type {materialImportSource.Material}");
            ...
            if (missingCountries.Count != 0)
                Base.Compute.RecordWarning($"No victims of modern slavery values available for the following countries {string.Join(", ", missingCountries)}, making up {missingRatio * 100}% of the {nameof(MaterialImportSources)} for the import country {materialImportSource.ImportCountry} for material of type {materialImportSource.Material}.");

            if (Math.Abs(1 - totalRatioUsed) > 0.01)   //Less than 99% used
                Base.Compute.RecordWarning($"The victims of modern slavery values returned from the {nameof(MaterialImportSources)} for the import country {materialImportSource.ImportCountry} for material of type {materialImportSource.Material} is based on a total of {totalRatioUsed * 100}% of the import ratios.");
